...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        s_lev=self.s_levels[level]

        for action in self.all_actions:
            a_node=PgNode_a(action)
        # 1. determine what actions to add and create those PgNode_a objects
            if a_node.prenodes.issubset(s_lev):
    ...
    def add_literal_level(self, level):
        ...
        nodes=set()
        prev_node=self.a_levels[level-1]
        for a_node in prev_node:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        set1=set(node_a1.action.effect_add)
        set2=set(node_a1.action.effect_rem)
        set3=set(node_a2.action.effect_add)
        set4=set(node_a2.action.effect_rem)

        # Check the intersection of two sets and see if there is any overlap
        return (len(set1.intersection(set4))+len(set2.intersection(set3)))>0        
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        set1=set(node_a1.action.effect_add)
        set2=set(node_a2.action.precond_neg)
        set3=set(node_a2.action.effect_add)
        set4=set(node_a1.action.precond_neg)
        set5=set(node_a1.action.effect_rem)
        set6=set(node_a2.action.precond_pos)
        set7=set(node_a2.action.effect_rem)
        set8=set(node_a1.action.precond_pos)

        if len(set1.intersection(set2))>0:
            return True
        if len(set3.intersection(set4))>0:
            return True
        if len(set5.intersection(set6))>0:
            return True
        if len(set7.intersection(set8))>0:
            return True
        return False
...
#        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            reached=False
            for level_ind,level in enumerate(self.s_levels):
                for s in level:
                    if reached:
                        break
                    if g==s.symbol:
